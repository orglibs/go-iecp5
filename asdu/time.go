...
// |         Milliseconds(D15--D8)       |
// | IV(D7)   RES1(D6)  Minutes(D5--D0)  | Minutes = 0-59, IV = invalid,0 = valid, 1 = invalid
...
// CP56Time2a 按 loc 编码七字节时间标签；nil 时区按 UTC 处理。
// isValid=false 时设置 IV 位；保留 circutor 的三参数签名。
// 星期和夏令时 SU 位均以目标时区为准，不能取转换时区前的原始日期。
func CP56Time2a(t time.Time, loc *time.Location, isValid bool) []byte {
	...
	return []byte{byte(msec), byte(msec >> 8), byte(ts.Minute()) | byte(boolToInt(!isValid)<<7), summerTimeHour(ts),
...
// 年份解释为 2000–2099；IV 置位或字段非法时返回零时间，不自动归一化非法日期。
...
// CP24Time2a 按 loc 编码三字节时间标签（毫秒和分钟，不包含小时和日期）。
