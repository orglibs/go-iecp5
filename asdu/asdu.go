...
type Params struct {
	...
	// size of ASDU common address.
	// Number of octets of the public address of the application service data unit.
	// The standard requires "a" in [1, 2].
	CommonAddrSize int

	// size of ASDU information object address.
...
// String Returns information about the identifier of the data unit： "TypeID Cause OrigAddr@CommonAddr"
...
func (sf *ASDU) ReplyCmd(coa CauseOfTransmission, addr CommonAddr, ioa InfoObjAddr, qoc QualifierOfCommand, val byte) *ASDU {
	sf.CommonAddr = addr
	r := NewASDU(sf.Params, sf.Identifier)
	r.Coa = coa
...
func (sf *ASDU) ReplySetPointCmd(coa CauseOfTransmission, addr CommonAddr, ioa InfoObjAddr, qos QualifierOfSetpointCmd, val int16) *ASDU {
	sf.CommonAddr = addr
	r := NewASDU(sf.Params, sf.Identifier)
	r.Coa = coa
